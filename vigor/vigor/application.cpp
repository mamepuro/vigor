...
namespace game {
	...
	bool Application::Run() {
		...
		while (glfwWindowShouldClose(window_) == GL_FALSE) {
			...
			ImGui::Begin("UI (operation) ");

			ImGui::Text("This is some useful text.");
			ImGui::Text("Current Block count %d", mesh_entities_.size());
			ImGui::DragFloat("scale", &scale, 0.1f, 0.1f, 3.f);
			ImGui::DragFloat("camera ", &yy);
			if (ImGui::Button("Add block"))
			{
				auto mesh = Mesh::LoadObjMesh("block.obj");
				auto block = new Block(mesh, glm::vec3(1.5, 1.5, 0.0f),
					glm::vec3(0.0f), glm::vec3(1.0f), bid, 1, mesh->vertices_,
					1, 5, 0, 3);
				bid++;
				mesh_entities_.emplace_back(*block);
			...
			for (auto&& mesh_entity : mesh_entities_)
			{
				mesh_entity.SetScale(glm::vec3(scale, scale, scale));
				mesh_entity.UpdateWorldPosition();
			}
		}
	...
	bool Application::PickBlock(float screen_x, float screen_y) {
		/*実装はhttps://ahbejarano.gitbook.io/lwjglgamedev/chapter-18を参照*/
		float normilize_x = (2 * screen_x) / 960 - 1; //960=width
		float normilize_y = 1 - (2 * screen_y) / 540;
		float normilize_z = -1; //デバイス正規化座標系のz座標は-1(無限遠)とする
		float normilize_w = 1; //デバイス正規化座標系のwは1とする
